...
def create_product_with_barcode(db: Session, product_data: dict):
    ...
    if db_product.barcode:
        barcode_generator.generate_barcode_for_product(db_product)

    return db_product


def update_product_barcode(db: Session, product_id: int, barcode: str):
    ...
    if product.barcode:
        barcode_generator.delete_barcode_image(product.barcode)
    ...
    if barcode:
        barcode_generator.generate_barcode_for_product(product)

    return product


def delete_product_and_barcode(db: Session, product_id: int):
    """Delete product and its barcode image"""
    product = get_product(db, product_id)
    if not product:
        return False
